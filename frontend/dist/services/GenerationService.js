...
const AIKanjiGenerationService_1 = require("./AIKanjiGenerationService");
class GenerationService {
    ...
    async generateKanjiName(sessionId) {
        ...
        if (answers.length < 14) {
            const error = new Error('Not all questions have been answered');
            error.code = 'INSUFFICIENT_ANSWERS';
            throw error;
        }
        // Calculate scoring
        const scoringResult = scoring_1.ScoringEngine.processAnswers(answers);
        ...
        const aiService = new AIKanjiGenerationService_1.AIKanjiGenerationService();
        const userProfile = {
     ...
     */
    async saveScoringResults(sessionId, scoringResult) {
        // Save gender profile
        const genderQuery = `
     ...
     */
    async saveAIResult(sessionId, kanjiResult, scoringResult) {
        const query = `
     ...
     */
    formatAIResult(kanjiResult, scoringResult, userName) {
        return {
     ...
     */
    async saveResult(sessionId, kanjiPair, scoringResult) {
     ...
     */
    formatNewResult(kanjiPair, scoringResult) {
        const kanjiName = kanjiPair.first_kanji.kanji_char + kanjiPair.second_kanji.kanji_char;
        return {
     ...
     */
    generateExplanation(kanjiPair, scoringResult, lang) {
        ...
        if (lang === 'ja') {
            return this.generateJapaneseAnalysis(genderDesc, personalityDesc.ja, motivation, secondMotivation, secondScore, genderTrait, kanjiPair, firstMeaning, secondMeaning);
        }
        else {
            return this.generateEnglishAnalysis(genderDesc, personalityDesc.en, motivation, secondMotivation, secondScore, genderTrait, kanjiPair, firstMeaning, secondMeaning);
        }
    }
    /**
     * Generate detailed Japanese personality analysis
     */
    generateJapaneseAnalysis(genderDesc, personalityCore, mainMotivation, secondMotivation, secondScore, genderTrait, kanjiPair, firstMeaning, secondMeaning) {
     ...
     */
    generateEnglishAnalysis(genderDesc, personalityCore, mainMotivation, secondMotivation, secondScore, genderTrait, kanjiPair, firstMeaning, secondMeaning) {
    ...
    getMotivationEssence(motivation) {
        const essences = {
    ...
    getMotivationEssenceEn(motivation) {
        const essences = {
    ...
    getInterpersonalStyle(genderTrait, motivation) {
        let base = '';
    ...
    getInterpersonalStyleEn(genderTrait, motivation) {
        let base = '';
        if (motivation === 'belonging') {
    ...
    getKanjiConnection(motivation, position) {
        const connections = {
    ...
    getKanjiConnectionEn(motivation, position) {
        const connections = {
