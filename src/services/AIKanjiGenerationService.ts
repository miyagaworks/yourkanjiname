...
5. 各漢字のmeaning_user: ${langConfig.name}での意味`;
  ...
  "meaning_user": "${langConfig.name}での意味（例：harmonie, paisible）",`;
    ...
    return `あなたは日本の名前の専門家です。外国人の性格・性質に基づいて、最適な漢字2文字の日本名を提案します。
...
# 最重要ルール：名前の音を漢字に当てはめないこと
【絶対厳守】ユーザーの名前（${userName}）の発音・音韻に合わせて漢字を選んではいけません。
例えば「Kent」に「健」「人」を当てる、「John」に「城」「応」を当てるなど、名前の音を日本語に翻訳・音訳するアプローチは禁止です。
漢字の選択は100%「性格分析・動機・行動特性」に基づいて行ってください。名前の発音は一切考慮しないでください。

# 性別に適した漢字選択（重要）
${genderKanjiGuidance}

# タスク
上記の**性格・動機・行動特性のみ**に基づいて、性別に適した漢字2文字の日本名を提案し、説明文を作成してください。名前の音は一切参考にしないこと。
...
2. **漢字の説明**:
   ...
   - 【重要】各漢字の説明には、必ずその漢字自体（例：「龍」「智」）を文中に明記して説明すること。漢字を伏せて「最初の文字は〜」のような曖昧な表現は禁止。必ず「一文字目の漢字は「龍」です。」「The first character "龍" (dragon)」のように漢字を直接示すこと。
